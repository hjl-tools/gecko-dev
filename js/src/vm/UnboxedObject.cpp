...
void UnboxedLayout::trace(JSTracer* trc) {
  ...
  if (newScript()) newScript()->trace(trc);
...
void UnboxedLayout::setNewScript(TypeNewScript* newScript,
  ...
  if (newScript_ && writeBarrier) TypeNewScript::writeBarrierPre(newScript_);
...
/* static */ bool UnboxedLayout::makeConstructorCode(JSContext* cx,
  ...
  masm.createGCObject(object, scratch1, templateObject, gc::TenuredHeap,
                      &failure,
                      /* initFixedSlots = */ false);
  ...
  for (size_t i = 0; i < layout.properties().length(); i++) {
    ...
    if (!UnboxedTypeNeedsPostBarrier(property.type)) continue;
  ...
  if (object.volatile_()) liveVolatileRegisters.add(object);
  ...
  for (size_t i = 0; i < layout.properties().length(); i++) {
    ...
    if (property.type == JSVAL_TYPE_OBJECT) {
      HeapTypeSet* types =
          group->maybeGetProperty(IdToTypeId(NameToId(property.name)));
...
void UnboxedLayout::detachFromCompartment() {
  if (isInList()) remove();
}
...
bool UnboxedPlainObject::containsUnboxedOrExpandoProperty(JSContext* cx,
                                                          jsid id) const {
  if (layoutDontCheckGeneration().lookup(id)) return true;
...
/* static */ bool UnboxedLayout::makeNativeGroup(JSContext* cx,
  ...
  if (layout.newScript()) {
    ...
    if (!replacementNewScript) return false;
  ...
  if (layout.allocationScript()) {
    ...
    cx->compartment()->objectGroups.replaceAllocationSiteGroup(
        script, pc, JSProto_Object, replacementGroup);

    // Clear any baseline information at this opcode which might use the old
    // group.
    if (script->hasBaselineScript()) {
      jit::ICEntry& entry =
          script->baselineScript()->icEntryFromPCOffset(script->pcToOffset(pc));
  ...
  ObjectGroup* nativeGroup = ObjectGroupCompartment::makeGroup(
      cx, &PlainObject::class_, proto,
  ...
  if (!group->unknownProperties()) {
    for (size_t i = 0; i < layout.properties().length(); i++) {
      ...
      if (!PropagatePropertyTypes(cx, id, group, nativeGroup)) return false;

      // If we are OOM we may not be able to propagate properties.
      if (nativeGroup->unknownProperties()) break;
...
/* static */ NativeObject* UnboxedPlainObject::convertToNative(JSContext* cx,
  ...
  if (!layout.nativeGroup()) {
    if (!UnboxedLayout::makeNativeGroup(cx, obj->group())) return nullptr;

    // makeNativeGroup can reentrantly invoke this method.
    if (obj->is<PlainObject>()) return &obj->as<PlainObject>();
  }
  ...
  if (!expando) return &obj->as<PlainObject>();
  ...
  for (size_t i = 0; i < ids.length(); i++) {
    id = ids[i];
    if (!GetOwnPropertyDescriptor(cx, nexpando, id, &desc)) return nullptr;
    ObjectOpResult result;
    if (!DefineProperty(cx, nobj, id, desc, result)) return nullptr;
...
/* static */ JSObject* UnboxedPlainObject::createWithProperties(
  ...
  if (!cx->helperThread() && !group->unknownProperties() &&
      !layout.constructorCode() && cx->runtime()->jitSupportsFloatingPoint &&
      jit::CanLikelyAllocateMoreExecutableMemory()) {
    if (!UnboxedLayout::makeConstructorCode(cx, group)) return nullptr;
  }
#endif
...
/* static */ bool UnboxedPlainObject::obj_defineProperty(
  ...
  if (const UnboxedLayout::Property* property = layout.lookup(id)) {
    ...
    if (!convertToNative(cx, obj)) return false;
...
/* static */ bool UnboxedPlainObject::obj_getOwnPropertyDescriptor(
  ...
  if (UnboxedExpandoObject* expando =
    ...
    if (expando->containsShapeOrElement(cx, id)) {
      RootedObject nexpando(cx, expando);
      if (!GetOwnPropertyDescriptor(cx, nexpando, id, desc)) return false;
      if (desc.object() == nexpando) desc.object().set(obj);
      return true;
    }
  }
...
/* static */ bool UnboxedPlainObject::newEnumerate(JSContext* cx,
  ...
  for (size_t i = 0; i < unboxed.length(); i++) {
    if (!properties.append(NameToId(unboxed[i].name))) return false;
  }
...
static bool UnboxedTypeIncludes(JSValueType supertype, JSValueType subtype) {
  if (supertype == JSVAL_TYPE_DOUBLE && subtype == JSVAL_TYPE_INT32)
    return true;
  if (supertype == JSVAL_TYPE_OBJECT && subtype == JSVAL_TYPE_NULL) return true;
  return false;
}
...
static bool CombineUnboxedTypes(const Value& value, JSValueType* existing) {
  ...
  if (UnboxedTypeIncludes(*existing, type)) return true;
  return false;
}
...
static bool CombinePlainObjectProperties(
  ...
  for (size_t i = 0; i < templateShape->slotSpan(); i++) {
    ...
    if (!CombineUnboxedTypes(val, &existing)) return false;
  }
...
static size_t ComputePlainObjectLayout(
    JSContext* cx, Shape* templateShape,
    UnboxedLayout::PropertyVector& properties) {
  ...
  for (size_t i = 0; i < ArrayLength(typeSizes); i++) {
    ...
    for (size_t j = 0; j < templateShape->slotSpan(); j++) {
      if (properties[j].offset != UINT32_MAX) continue;
...
static bool SetLayoutTraceList(JSContext* cx, UnboxedLayout* layout) {
  ...
  for (size_t i = 0; i < layout->properties().length(); i++) {
    ...
    if (property.type == JSVAL_TYPE_OBJECT) {
      if (!objectOffsets.append(property.offset)) return false;
    } else if (property.type == JSVAL_TYPE_STRING) {
      if (!stringOffsets.append(property.offset)) return false;
    }
  }
...
static bool GetValuesFromPreliminaryPlainObject(
  ...
  for (size_t i = 0; i < obj->slotSpan(); i++) {
    if (!values.append(obj->getSlot(i))) return false;
  }
  return true;
}
...
bool js::TryConvertToUnboxedLayout(JSContext* cx, AutoEnterAnalysis& enter,
  ...
  if (jit::JitOptions.disableUnboxedObjects) return true;
  ...
  if (templateShape->slotSpan() == 0) return true;
  ...
  for (size_t i = 0; i < PreliminaryObjectArray::COUNT; i++) {
    ...
    if (obj->isSingleton() || obj->group() != group) return true;
  ...
  for (size_t i = 0; i < templateShape->slotSpan(); i++) {
    ...
    if (UnboxedTypeSize(properties[i].type) == 0) return true;
  }
  ...
  for (Shape::Range<NoGC> r(templateShape); !r.empty(); r.popFront()) {
    ...
    uint32_t dummy;
    if (!JSID_IS_ATOM(id) || JSID_TO_ATOM(id)->isIndex(&dummy)) return true;
  }
